...
PopInTownControllers.controller('MainCtrl', ['$scope', '$location', '$window', 'ProfileService', '$sce', '$upload',
	...
	$scope.init = function (data) {
		...
		$scope.initMCE();
		$scope.initMceEdu();
		$scope.initMceBasicInfo();
		$scope.educationLevel = $scope.data.educationLevels[0];
		//$scope.expEdit.dateStarted.year = $scope.data.years[0];
		//$scope.expEdit.dateStarted.month = $scope.data.months[4];
	};

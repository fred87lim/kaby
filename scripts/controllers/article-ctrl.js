...
phonecatServices.factory('ProfileService', function ($http) {
	return {
		...
		findGeneralSettings: function () {
			return $http.get('/api/v2/settings/general');
		},
...
PopInTownControllers.controller('MainCtrl', ['$scope', '$location', '$window', 'ProfileService', '$sce', '$upload', 
	...
	$scope.init = function () {
		...
		ProfileService.findGeneralSettings().success(function (result) {
    		console.log(result);
    		if (result.status) {
    			$scope.settings = result.data;
    			$scope.selectedCategory = result.data.categories[0];
    			$scope.selectedPrivacy = result.data.privacies[0];

    			$scope.article.category = result.data.categories[0];
    			$scope.article.privacy = result.data.privacies[0];
	...
	$scope.autosave = function () {
		...
		if(tinyMCE.activeEditor.isDirty()) {
		    ...
		    var content = tinyMCE.get('article-editor');
		    var notDirty = tinyMCE.get('article-editor');
		    content = (tinyMCE.activeEditor.getContent());

		    if ($scope.article.content != content) {
		    	$scope.article.content = content;
		    	$scope.article.lastSaved = new Date();
		    	console.log(content);
		    }
	...
	/*
	 *	When user select a category from dropdown list
	 */
	$scope.selectCategory = function (cat) {
		$scope.selectedCategory = cat;
		$scope.article.category = cat;
	};

	/*
	 *	When user select a category from dropdown list
	 */
	$scope.selectPrivacy = function (cat) {
		$scope.selectedPrivacy = cat;
		$scope.article.privacy = cat;
	};

...
phonecatApp.directive( 'compileData', function ( $compile ) {
  return {
    scope: true,
      ...
      attrs.$observe( 'template', function ( myTemplate ) {
        if ( angular.isDefined( myTemplate ) ) {
          ...
          elmnt = $compile( myTemplate )( scope );
            ...
            element.html("<p>This is another test</p>"); // dummy "clear"
            console.log(elmnt);
            console.log(element);
            console.log(attrs);
...
phonecatApp.directive("addbuttonsbutton", function(){
	return {
		restrict: "E",
...
phonecatApp.directive("addbuttons", function($compile){
...
phonecatApp.directive("alert", function(){
	return function(scope, element, attrs){
		element.bind("click", function(){
			console.log(attrs);
...
phonecatServices.factory('tempService', function () {
  return function () { 
    return '<p>Test</p>';
  };
});
...
phonecatServices.factory('ProfileService', function ($http) {
	return {
		...
		findArticle: function (username, articleSlug) {
			return $http.get('/ajax/' + username + '/articles/' + articleSlug);
		},
		likeArticle: function (inputs) {
			return $http.post('/ajax/like/article', inputs);
		},
		unlikeArticle: function (inputs) {
			return $http.post('/ajax/unlike/article', inputs);
		},
		commentArticle: function (inputs) {
			return $http.post('/ajax/article/comment', inputs);
		},
		likeComment: function (inputs) {
			return $http.post('/ajax/comment/like', inputs);
		},
		unlikeComment: function (inputs) {
			return $http.post('/ajax/comment/unlike', inputs);
		},
...
PopInTownControllers.controller('MainCtrl', ['$scope', '$location', '$window', 'ProfileService', '$sce', '$upload', 
	'$filter', '$http', '$interval', 'tempService', '$compile',
	function ($scope, $location, $window, ProfileService, $sce, $upload, $filter, $http, $interval, tempService, $compile) {
	...
	$scope.init = function (username, articleSlug) {
		...
		$scope.user.articleSlug = articleSlug;
  ...
  //   	editor.setFontSize($scope.codeHighlight.size.value);
		...
		//$scope.initMceArticle();
		$scope.initMce();

		tinymce.execCommand('mceFocus',true,'article-editor');
		...
		// ProfileService.textEditorTemplate('test').success(function (result) {
		// 	console.log(result);
		// 	$scope.newTransaction = result;
		// });
	};

	$scope.editorHtml = '<textarea id=\'article-editor\' class=\'mceArticleEditor\' style=\'height: 300px\'></textarea>' +
		'</br>' + '<button type="button" class="btn btn-primary btn-block" ng-click="replyComment()">Reply</button>';

	$scope.commentEditorFocus = '<textarea class="form-control" rows="3" placeholder="Comment..." ng-focus="showCommentTextEditor()"></textarea>';

	// To keep track what comment is going to be replied.
	$scope.activeReplyEditor = null;

	$scope.replyComment = function () {

		if (!$scope.activeReplyEditor) {
			return;
		}

		var content = tinyMCE.activeEditor.getContent();
		var object = {
			commentId: $scope.activeReplyEditor._id,
			content: content
		}
		console.log($scope.activeReplyEditor);
		console.log(object);

		// Post comment reply
		ProfileService.replyComment(object).success(function (result) {
			if (result.status) {
				console.log(result);
			} else {
	...
	$scope.showReplyTextEditor = function (comment) {
		// remove all text editor of other elements
		$('.comments-editor').html('');
		var value = $compile(angular.element($scope.editorHtml))($scope);

		$scope.activeReplyEditor = comment;

		var commentEditorValue = $compile(angular.element($scope.commentEditorFocus))($scope);
		$('#article-comment').append(commentEditorValue);
		$('#comment-' + comment._id).append(value);

		$scope.initMce();
	}

	$scope.showCommentTextEditor = function () {
		$('.comments-editor').html('');
		$('#article-comment').append('<textarea id=\'article-editor\' class=\'mceArticleEditor\' style=\'height: 300px\'></textarea>');
		$scope.initMce();
	}
	...
	$scope.likeComment = function (comment) {
		...
		if (comment.commentLiked) {
			// Unlike comment
			ProfileService.unlikeComment(object).success(function (result) {
				if (result.status) {
					console.log(result);

					// Loop through all comments
					for (var i = 0; i < $scope.user.article.comments.length; i++) {
						if ($scope.user.article.comments[i]._id == comment._id) {

							// Check which comment of this like
							// remove like from comment's like collection
			    			var index = -1;
			    			for (var j = 0; j < $scope.user.article.comments[i].commentLikes.length; j++) {
			    				if ($scope.user.article.comments[i].commentLikes[j]._id == result.data._id) {
			    					index = j;
			    					break;
			    				}
			    			}
			    			...
			    			if (index != -1) {
			    				$scope.user.article.comments[i].commentLikes.splice(index, 1);
			    				$scope.user.article.comments[i].commentLiked = false;
			    			}
			...
			// Like comment
			ProfileService.likeComment(object).success(function (result) {
				if (result.status) {
					console.log(result);

					// add like to comment's like collection
					// Loop through all comments
					for (var i = 0; i < $scope.user.article.comments.length; i++) {
						if ($scope.user.article.comments[i]._id == comment._id) {
							$scope.user.article.comments[i].commentLikes.push(result.data);
							$scope.user.article.comments[i].commentLiked = true;
							break;
						}
					}
		...
		if (reply.liked) {
			...
			ProfileService.unlikeComment(object).success(function (result) {
				if (result.status) {
					...
					// Loop through all comments
					for (var i = 0; i < $scope.user.article.comments.length; i++) {
						var comment = $scope.user.article.comments[i];
						var hasFound = false;
						// Loop through all replies
						for (var n = 0; n < comment.replies.length; n++) {
							if (comment.replies[n]._id == reply._id) {
								// Check which comment of this like
								// remove like from comment's like collection
								
				    			var index = -1;
				    			for (var j = 0; j < comment.replies[n].likes.length; j++) {
				    				if (comment.replies[n].likes[j]._id == result.data._id) {
				    					index = j;
				    					break;
				    				}
				    			}
				    			...
				    			if (index != -1) {
				    				$scope.user.article.comments[i].replies[n].likes.splice(index, 1);
				    				$scope.user.article.comments[i].replies[n].liked = false;
				    				var hasFound = true;
				    				break;
				    			}
				...
				if (result.status) {
					...
					for (var i = 0; i < $scope.user.article.comments.length; i++) {
						...
						for (var j = 0; j < comment.replies.length; j++) {
							if (comment.replies[j]._id == reply._id) {
								$scope.user.article.comments[i].replies[j].likes.push(result.data);
								$scope.user.article.comments[i].replies[j].liked = true;
								hasFound = true;
								break;
							}
						}
	...
	$scope.commentArticle = function () {
		...
		var content = tinyMCE.activeEditor.getContent();
		var object = {
			articleId: $scope.user.article._id,
			content: content
		}

		ProfileService.commentArticle(object).success(function (result) {
			if (result.status) {
				console.log(result);
			} else {
	...
	$scope.likeUnlikeArticle = function (articleId) {
		var object = {
			articleId: articleId
		};

		if ($scope.user.article.liked == true) {
			ProfileService.unlikeArticle(object).success(function (result) {
	    		if (result.status) {
	    			$scope.user.article.liked = false;

	    			// remove user's like from list of likes
	    			var index = -1;
	    			for (var i = 0; i < $scope.user.article.likes.length; i++) {
	    				if ($scope.user.article.likes[i]._id == result.data._id) {
	    					index = i;
	    					break;
	    				}
	    			}
	    			...
	    			if (index != -1) {
	    				$scope.user.article.likes.splice(index, 1);
	    			}
	    		} else {
			...
			ProfileService.likeArticle(object).success(function (result) {
	    		if (result.status) {
	    			$scope.user.article.liked = true;
	    			$scope.user.article.likes.push(result.data);
	    		} else {
	...
	$scope.findArticle = function () {
		ProfileService.findArticle($scope.user.username, $scope.user.articleSlug).success(function (result) {
    		console.log(result);
    		if (result.status) {
    			$scope.user.article = result.data;
    		} else {
